...
public class AnakartDAO extends DBConnection {
    ...
    public List<Anakart> read() {
        ...
        try {
            ...
            while (rs.next()) {
                Anakart tmp = new Anakart(rs.getInt("anakart_id"), rs.getString("anakart_adı"), rs.getString("uyumlu_kasa"), rs.getString("soket_turu"), rs.getString("bellek_turu"), rs.getInt("maxbellek"),rs.getInt("anakart_fiyat"));
                list.add(tmp);
